<form method="get" id="filter_form_panel" class="mb-0" action="{{ route('filter') }}">
            ...
            @foreach($eachBrands as $data)
                ...
                <li>
                    <div class='d-flex rounded px-1'>
                        <input class='form-check-input me-1 checkedList' type='checkbox' name='brand[]' value='{{ $data->brand }}' id='{{ $data->brand }}' {{ $checked_brand }}>
                        <label class='w-100 link-body-emphasis' for='{{ $data->brand }}' style='cursor: pointer;'>{{ $data->brand }}</label>
                    </div>
                </li>
